...
def load_config(filepath):
    ...
    if not os.path.exists(filepath):
        sys.exit("{} does not exist".format(filepath))
...
def api_list_files_in_dataset(token, dataset):
    ...
    if(dataset not in api_list_authorized_datasets(token)):
...
def pretty_print_files_in_dataset(reply, dataset):
    ...
    for res in reply:
        print(format_string.format(res['fileId'], status_ok(res['fileStatus']), str(res['fileSize']), res['checksum'],
                                   res['fileName']))
...
def calculate_md5(fname):
    if not os.path.exists(fname):
        raise Exception("Downloaded file '{}' does not exist".format(fname))
...
def generate_output_filename(folder, file_id, file_name, genomic_range_args):
    ...
    if is_genomic_range(genomic_range_args):
        ...
        if formatExt != ext and len(formatExt) > 1:
            ext += formatExt
...
def download_file(token, file_id, file_size, check_sum, num_connections, key, output_file=None):
    ...
    with tqdm(total=int(file_size), unit='B', unit_scale=True) as pbar:
        params = [
            (url, token, output_file, chunk_start_pos, min(chunk_len, file_size-chunk_start_pos), pbar)
    ...
    if (received_file_md5 == check_sum or not_valid_server_md5):
        ...
        if not_valid_server_md5:
            logging.info("WARNING: Unable to obtain valid MD5 from the server(recived:{}). "
                         "Can't validate download. Contact EGA helpdesk".format(check_sum))
...
def download_dataset(credentials,  dataset_id, num_connections, key, output_dir,
                     genomic_range_args, max_retries, retry_wait):
    ...
    for res in reply:
        try:
            if (status_ok(res['fileStatus'])):
                output_file = None if (output_dir is None) else generate_output_filename(
                    output_dir, res['fileId'], res['fileName'], genomic_range_args)
                download_file_retry(
                    credentials, res['fileId'], res['fileName'], res['fileSize'], res['checksum'],
...
def main():
    ...
    parser.add_argument("-cf", "--config-file", required=True,
                        help="JSON file containing credentials/config e.g.{'username':'user1','password':'toor',"
                             "'key': 'abc','server_url': 'https://ega.ebi.ac.uk:8051/elixir/data'}")
    ...
    *credentials, key = load_config(args.config_file)
    ...
    if args.subcommand == "files":
        ...
        if (args.identifier[3] == 'D'):
            download_dataset(
                credentials, args.identifier, args.connections,
                key, args.saveto, genomic_range_args,
            ...
            download_file_retry(
                credentials, args.identifier, file_name, file_size, check_sum,
                args.connections, key, args.saveto, genomic_range_args, args.max_retries, args.retry_wait)
        else:
